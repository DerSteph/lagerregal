...
public class UmlagernWindowNew extends JFrame {
	...
	public UmlagernWindowNew() {
		...
		for (int i = 0; i < 27; i++) {
			...
			label[i].addActionListener(new java.awt.event.ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					// Unterschiedliche Zustände, je nach dem ob ein Objekt zum Auslagern ausgewählt
					// wurde
					if (modus == "Lagerquelle") {
						modus = "Lagerziel";
						lagerquelle = Start.window.getLagerplatzZuGrafiklagerplatz(temp);
						checkliste = Start.lager
								.getArrayVonFreienPlaetzen(Start.window.getLagerplatzZuGrafiklagerplatz(temp));
						...
						if (Start.lager.Umlagern(lagerquelle, lagerziel)) {
							...
							System.out.println(Start.window.getGrafikLagerplatzZuLagerplatz(lagerquelle) + ","
									+ Start.window.getGrafikLagerplatzZuLagerplatz(lagerziel));
							Start.window.letzteAktion.setText("Letzte Aktion: Umlagerung von "
									+ Start.lager.getLagerplatzInhalt(
											Start.window.getLagerplatzZuGrafiklagerplatz(temp))
									+ " von Lager "
									+ Start.window.getLagerplatzFromInhalt(Start.window.getRightLagerplatz(lagerquelle))
									+ " zu Lager "
									+ Start.window.getLagerplatzFromInhalt(Start.window.getRightLagerplatz(lagerziel)));
		...
		lagertext.setText(
				"<html><center>W\u00E4hle ein Produkt aus, was du umlagern m\u00F6chtest.<br><font color='red'>Umlagern kostet <b>100\u20AC</b></font></center></html>");
