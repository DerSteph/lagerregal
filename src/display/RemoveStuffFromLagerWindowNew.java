...
public class RemoveStuffFromLagerWindowNew extends JFrame {
	...
	public RemoveStuffFromLagerWindowNew(int num) {
		...
		// Um Zählen wv Plätze verfügbar sind oder blockiert sind für das Produkt zum
		// Rausholen
		int j = 0;
		int verfuegbar = 0;
		int blockiert = 0;
		for (int i = 0; i < 27; i++) {
			...
			Boolean check = Start.lager.checkObProduktDortGelagert(Start.window.getLagerplatzZuGrafiklagerplatz(i),
					Start.lager.getAuftrag(num));
			...
			label[i].addActionListener(new java.awt.event.ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					if (Start.lager.getAuftrag(num) == null) {
						...
						// Führe Auftrag aus
						String text = Start.lager
								.getLagerplatzInhalt(Start.window.getLagerplatzZuGrafiklagerplatz(temp));
						if (Start.lager.auftragAbarbeiten(num, Start.window.getLagerplatzZuGrafiklagerplatz(temp))) {
							...
							Start.window.letzteAktion.setText("Letzte Aktion: Auslagern von " + text + " in Lager "
									+ Start.window.getLagerplatzFromInhalt(temp));
							dispose();
						} else {
							JOptionPane.showMessageDialog(null,
									"<html><body><center>Der Lagerplatz ist bereits durch andere Produkte davor blockiert!<br> Versuche mit der Funktion <font color='red'>Umlagern</font> die davorstehenden Produkte umzulagern<br>oder mit der Funktion <font color='red'>Verschrotten</font> die davorstehenden Produkte zu verschrotten.</center></body></html>",
		...
		if (verfuegbar == 0 && blockiert == 0) {
			lagertext.setText("<html><center>" + Start.lager.getAuftrag(num).getInhalt()
					+ "<br><font color='red'>Das Produkt ist nicht im Lager!<br>Notfalls kannst du den Auftrag<br>auch zurückstellen oder ablehnen.</font></center></html>");
		} else {
			if (verfuegbar != 0) {
				lagertext.setText("<html><center>" + Start.lager.getAuftrag(num).getInhalt() + "<br>Verfügbar: "
						+ verfuegbar + " mal<br>" + "Blockiert: " + blockiert + " mal</center></html>");
			} else {
				lagertext.setText("<html><center>" + Start.lager.getAuftrag(num).getInhalt()
						+ "<br><font color='red'>Alle " + blockiert
						+ " Produkte werden blockiert. <br>Durch Umlagern anderer Produkte<br>kannst du sie freischaufeln.</font></center></html>");
			}
		}
