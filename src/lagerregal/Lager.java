...
public class Lager {
	...
	public boolean auftragAbarbeiten(int auswahl, int lagerplatz) {
		...
		if (((Produkt) auftrag).getLagerungsart().equals("Einlagerung")) {
			if (auftrag instanceof Papier) {
				...
				if (((Holz) auftrag).getForm().equals("Balken")) {
					...
					if (lagerinhalt.get(k) == null && lagerinhalt.get(k - 3) == null
							&& lagerinhalt.get(k - 6) == null) {
		...
		// Hauptsächliche Prüfung, ob das Produkt dem ausgewählten Produkt entspricht
		// auf dem Lagerplatz
			...
			if (auftrag instanceof Papier) {
				if (lagerinhalt.get(lagerplatz) instanceof Papier) {
					if (((Papier) lagerinhalt.get(lagerplatz)).getFarbe().equals(((Papier) auftrag).getFarbe())) {
						if (((Papier) lagerinhalt.get(lagerplatz)).getGroesse()
								.equals(((Papier) auftrag).getGroesse())) {
		...
		if (name != null) {
			if (name instanceof Papier) {
				...
				for (int i = 26; i > 0; i--) {
					...
					// Wenn Lagerplatz der abgefragte Gegenstand ist -> bei nächsten Schritt die
					// hinteren Plätze blockieren
			...
			if (name instanceof Holz) {
				...
				if (((Holz) name).getForm().equals("Balken")) {
					...
					for (int j = 0; j < 3; j++) {
						for (int i = 0; i < 3; i++) {
							if (lagerinhalt.get(i + k) != null || lagerinhalt.get(i + k + 3) != null
									|| lagerinhalt.get(i + k + 6) != null) {
								checkliste[i + k] = 0;
								checkliste[i + k + 3] = 0;
								checkliste[i + k + 6] = 0;
							}
	...
	public int getAnzahlFreierVerfuegbarerPlaetze(Produkt name) {
		// 0 = belegt, 1 = frei, 2 = ausgewaehlter gegenstand
		// Technik genau gleich wie bei getArrayVonFreienPlaetzen, hier werden die
		// Plaetze aber gezaehlt
		int checkliste[] = new int[27];
		int ohneAbfrage = -1;
		for (int i = 0; i < checkliste.length; i++) {
			checkliste[i] = 1;
	...
	// Prüft, wo Plätze im Lager zum Umlagern verfügbar sind, gibt es als Array
	// zurück
	...
	public boolean Umlagern(int lagerquelle, int lagerziel) {
		...
		if (lagerinhalt.get(lagerquelle) instanceof Holz) {
			if (((Holz) lagerinhalt.get(lagerquelle)).getForm().equals("Balken")) {
				...
				if (lagerinhalt.get(holz_lagerziel) == null && lagerinhalt.get(holz_lagerziel - 3) == null
						&& lagerinhalt.get(holz_lagerziel - 6) == null) {
	...
	// Bei Holz soll immer die vorderste Position ausgewählt werden, zum Ein und
	// Auslagern, daher die Funktion
